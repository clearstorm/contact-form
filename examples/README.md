...
| --- | --- |
| `/` | Index + quickstart |
| `/field-types` | **Field types** — four named forms from one file: the **all 20 field types** kitchen sink (rendering + every validation rule; checkbox groups, radio, single consent toggle, hidden passthrough, file uploads single + `multiple`, a multi-select, a `datalist`-suggesting text input; required and `optional: true` twins; per-field `message` overrides; `rf-span-*` column sizes), the **structural field types** (`heading`, `description`, `divider`, `section` — field types that render static markup, never reaching `data-rules` or the payload), the **validation extras** form (custom rules layered on any field: `pattern`, `minLength`/`maxLength` with live character counters, `sameAs`, `minSelect`/`maxSelect`) and the **repeat rows** form (bounded + unbounded `repeater` row groups; rows validate individually and submit as one JSON array per group) |
...
| `/wizard` | **Multi-step wizard (`step` markers)** — three named forms from one file: the flagship wizard (a conditional Company pane revealed by `account_type = "Business"` via marker-level `showWhen`, a centered rule-flanked stepper, pane-header defaults + per-marker overrides, step-scoped "Next" validation, a cross-step `showWhen` reveal, a hoisted hidden field, and `autoSave` progress drafts), every field type on an `even`+`center` stepper with a `background` band, and the compact chrome options (`nav` without numbers / jump-backs, custom "Continue" + `ghost` "Back", a `full` pane-header rule, a bare `show: false` pane) |
...
| --- | --- |
| `/` | Index + quickstart |
| `/field-types` | The all-20 field types kitchen sink + the structural field types + the repeat-rows form |
| `/conditional` | Realistic enquiry, all seven `showWhen` operators, and an AND reveal |
| `/wizard` | The three wizard forms from one spec file (step markers + conditional steps + stepper) |
...
| --- | --- |
| `wizard.json` | **`Flagship wizard`** — four `step` markers (only the last carries a `submit` label; the second, a conditional **Company** pane, is skipped while `account_type` isn't `"Business"` — hidden + inert, struck-through chip, out of validation and the payload), `submit`/`next`/`prev` as `{ label, variant }` button specs, `stepper` chrome (`nav` strip: variant/line/clickable; `header` pane-header defaults) with per-marker overrides, a shared prefix, a cross-step `showWhen` field, a hoisted hidden field, `autoSave` progress drafts and `"validateOn": "touched"` (don't-nag live validation: fields validate the first time they lose focus) · **`Every field type`** — 19 types across three panes with required + `optional: true` twins; `nav` `variant: "even"` + `line: "center"` + `background`; a cross-step `showWhen` and a hoisted hidden field · **`Compact chrome`** — `nav` without numbers / jump-backs, custom `next` label + `ghost` `prev`, a `full` pane-header rule and a bare `show: false` pane |
