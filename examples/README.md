...
| --- | --- |
| `/` | Index + quickstart |
| `/general` | **General forms** — three named forms from one file: a booking form with `prefill="datetime"` (date/time pre-filled yet still optional; required guests; extras folded into the payload), the **all 19 field types** kitchen sink (rendering + every validation rule; checkbox groups, radio, single consent toggle, hidden passthrough; required and `optional: true` twins; per-field `message` overrides; `rf-span-*` column sizes) and a **form structure** form (`heading`, `description`, `divider`, `section` — structural elements never reach `data-rules` or the payload) |
...
| `general.json` | **`Booking (prefill)`** — `prefill="datetime"` companion: optional date/time, required guests, optional message · **`All 19 field types`** — every core type; per-field `message` overrides; a form-level `copy` bag; a `hidden` field with a static value · **`Structure + decor`** — all four structural elements (`heading`, `description`, `divider`, `section`) interleaved with real fields; none of them reach `data-rules` or the payload |
