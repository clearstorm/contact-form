...
| `/wizard` | **Multi-step wizard (`step` markers)** — three steps with a centered rule-flanked stepper and Back/Next footer; a pane header per step (title on step 2, rule-left/title-right override on step 3), completed steps clickable to jump back, a shared prefix, a cross-step `showWhen` reveal (step 2's checkbox drives a step 3 field), a hoisted hidden field, and step-scoped “Next” validation |
...
| `wizard.json` | **Multi-step wizard** — three `step` markers (only the last carries a `submit` label), `submit`/`next`/`prev` as `{ label, variant }` button specs, `stepper` chrome (`nav` strip: variant/line/clickable; `header` pane-header defaults) with per-marker overrides, a shared prefix, a cross-step `showWhen` field, and a hoisted hidden field |
